...
namespace Honoo.MangaPack
{
    internal class Repack
    {
        ...
        Start:
            ...
            Console.WriteLine();
            Console.ForegroundColor = ConsoleColor.Green;
            Console.WriteLine("    1. ZIP/RAR/7Z 保留原有文件夹结构，PDF 内容保存在根文件夹");
            ...
            while (true)
            {
                ...
                switch (kc)
                {
                    ...
                    case '1': mode = 1; modeInfo = "1. ZIP/RAR/7Z 保留原有文件夹结构，PDF 内容保存在根文件夹"; break;
                    case '2': mode = 2; modeInfo = "2. 添加一层同名外包围文件夹"; break;
        ...
        private static void Do(string path, int mode, IList<string> log, ref int error)
        {
            if (File.Exists(path))
            {
                ...
                else if (status == UnpackStatus.IsEmpty)
                {
                    log.Add($"文件 {path} 是空压缩包。");
                    error++;
                }
                else
                {
                                                  ...
                                                  out IList<EntrySettings> entries)
        {
            ...
            if (entries.Count > 0)
            {
                entries = SetNewKeys(entries, string.Empty, main, mode);
                    ...
                    if (archive.TotalUncompressSize > 200 * 1024 * 1024)
                    {
                        ...
                        if (Common.IgnoreThumbs)
                        {
                            ...
                            foreach (IArchiveEntry entry in archive.Entries)
                            {
                                ...
                                {
                                    if (!entry.Key.EndsWith("Thumbs.db", StringComparison.OrdinalIgnoreCase))
                                    {
                                        string key = entry.Key.Replace('/', '\\');
                                        string temp = Path.Combine(tempDir, i.ToString());
                                        entry.WriteToFile(temp);
                                        entries.Add(new EntrySettings(key, temp, null));
                                        i++;
                                    }
                                }
                            }
                            ...
                            foreach (IArchiveEntry entry in archive.Entries)
                            {
                                if (!entry.IsDirectory)
                                {
                                    if (!entry.Key.EndsWith("Thumbs.db", StringComparison.OrdinalIgnoreCase))
                                    {
                                        ...
                                        entries.Add(new EntrySettings(key, null, temp));
                                    }
                                }
                            }
                        }
        ...
        private static IList<EntrySettings> SetNewKeys(IList<EntrySettings> entries, string root, string main, int mode)
        {
            switch (mode)
            {
                case 4:
                    if (root.Length == 0)
                    {
                        return SetNewKeys(entries, main, main, mode);
                    }
                    else
                    {
                        bool same = true;
                        ...
                        {
                            int remove = root.Length == main.Length ? 0 : root.Length - 1 - main.Length;
                            foreach (var entry in entries)
                            {
