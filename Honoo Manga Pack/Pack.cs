    ...
    internal class Pack
    {
        ...
        internal static void Work()
        {
            while (true)
            {
                ...
                Console.WriteLine();
                Console.ForegroundColor = ConsoleColor.Green;
                Console.WriteLine("    1. 从拖放文件夹的次级内容开始打包                                               Q. 忽略 Thumbs.db 文件 -- {0}", Common.IgnoreThumbs);
                Console.WriteLine();
                ...
                while (true)
                {
                    ...
                    switch (kc)
                    {
                        ...
                        case '1': Do(1, "1. 从拖放文件夹的次级内容开始打包"); break;
                        case '2': Do(2, "2. 从拖放文件夹自身开始打包"); break;
            ...
            if (Directory.Exists(path))
            {
                ...
                if (!string.IsNullOrEmpty(parent))
                {
                    ...
                    if (files.Length > 0)
                    {
                        ...
                        {
                            if (file.EndsWith("Thumbs.db", StringComparison.OrdinalIgnoreCase) && Common.IgnoreThumbs)
                            {
                                continue;
                            }
                            entries.Add(new EntrySettings(file.Replace('/', '\\'), file, null, null));
                        }
                        if (entries.Count > 0)
                        {
